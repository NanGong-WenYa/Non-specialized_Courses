// modules are defined as an array
// [ module function, map of requires ]
//
// map of requires is short require name -> numeric require
//
// anything defined in a previous bundle is accessed via the
// orig method which is the require for previous bundles
parcelRequire = (function (modules, cache, entry, globalName) {
  // Save the require from previous bundle to this closure if any
  var previousRequire = typeof parcelRequire === 'function' && parcelRequire;
  var nodeRequire = typeof require === 'function' && require;

  function newRequire(name, jumped) {
    if (!cache[name]) {
      if (!modules[name]) {
        // if we cannot find the module within our internal map or
        // cache jump to the current global require ie. the last bundle
        // that was added to the page.
        var currentRequire = typeof parcelRequire === 'function' && parcelRequire;
        if (!jumped && currentRequire) {
          return currentRequire(name, true);
        }

        // If there are other bundles on this page the require from the
        // previous one is saved to 'previousRequire'. Repeat this as
        // many times as there are bundles until the module is found or
        // we exhaust the require chain.
        if (previousRequire) {
          return previousRequire(name, true);
        }

        // Try the node require function if it exists.
        if (nodeRequire && typeof name === 'string') {
          return nodeRequire(name);
        }

        var err = new Error('Cannot find module \'' + name + '\'');
        err.code = 'MODULE_NOT_FOUND';
        throw err;
      }

      localRequire.resolve = resolve;
      localRequire.cache = {};

      var module = cache[name] = new newRequire.Module(name);

      modules[name][0].call(module.exports, localRequire, module, module.exports, this);
    }

    return cache[name].exports;

    function localRequire(x){
      return newRequire(localRequire.resolve(x));
    }

    function resolve(x){
      return modules[name][1][x] || x;
    }
  }

  function Module(moduleName) {
    this.id = moduleName;
    this.bundle = newRequire;
    this.exports = {};
  }

  newRequire.isParcelRequire = true;
  newRequire.Module = Module;
  newRequire.modules = modules;
  newRequire.cache = cache;
  newRequire.parent = previousRequire;
  newRequire.register = function (id, exports) {
    modules[id] = [function (require, module) {
      module.exports = exports;
    }, {}];
  };

  var error;
  for (var i = 0; i < entry.length; i++) {
    try {
      newRequire(entry[i]);
    } catch (e) {
      // Save first error but execute all entries
      if (!error) {
        error = e;
      }
    }
  }

  if (entry.length) {
    // Expose entry point to Node, AMD or browser globals
    // Based on https://github.com/ForbesLindesay/umd/blob/master/template.js
    var mainExports = newRequire(entry[entry.length - 1]);

    // CommonJS
    if (typeof exports === "object" && typeof module !== "undefined") {
      module.exports = mainExports;

    // RequireJS
    } else if (typeof define === "function" && define.amd) {
     define(function () {
       return mainExports;
     });

    // <script>
    } else if (globalName) {
      this[globalName] = mainExports;
    }
  }

  // Override the current require with this new one
  parcelRequire = newRequire;

  if (error) {
    // throw error from earlier, _after updating parcelRequire_
    throw error;
  }

  return newRequire;
})({"scatterPlot.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;
function scatterPlot() {
  var data = [{
    'income': 108400000.0,
    'views': 228000000000.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 64700000.0,
    'views': 28368841870.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 94800000.0,
    'views': 164000000000.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 87500000.0,
    'views': 148000000000.0,
    'category': 'Shows',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 35100000.0,
    'views': 93247040539.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1900000.0,
    'views': 29058044447.0,
    'category': 'Gaming',
    'population': 126226568.0,
    'Unemployment_rate': 2.29,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 63.2
  }, {
    'income': 2300000.0,
    'views': 90479060027.0,
    'category': 'People & Blogs',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 27900000.0,
    'views': 77180169894.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 38600000.0,
    'views': 57856289381.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 34300000.0,
    'views': 77428473662.0,
    'category': 'Sports',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 23900000.0,
    'views': 32144597566.0,
    'category': 'People & Blogs',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 79600000.0,
    'views': 101000000000.0,
    'category': 'Shows',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8100000.0,
    'views': 20826993957.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 8500000.0,
    'views': 30608119724.0,
    'category': 'Music',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 28700000.0,
    'views': 28634566938.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 81900000.0,
    'views': 73139054467.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 22700000.0,
    'views': 38843229963.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 20200000.0,
    'views': 45757850229.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 12200000.0,
    'views': 28648024439.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 57000000.0,
    'views': 61510906457.0,
    'category': 'Shows',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6800000.0,
    'views': 16241549158.0,
    'category': 'Sports',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5500000.0,
    'views': 59316472754.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 20300000.0,
    'views': 33431802698.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 29400000.0,
    'views': 57271630846.0,
    'category': 'Music',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 11100000.0,
    'views': 40602020243.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 22100000.0,
    'views': 25307753534.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 12400000.0,
    'views': 28837144516.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 12500000.0,
    'views': 27073872856.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5400000.0,
    'views': 14696003229.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 24600000.0,
    'views': 35302243691.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 9700000.0,
    'views': 30367676736.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 39200000.0,
    'views': 30516172739.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 25400000.0,
    'views': 29884657286.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7700000.0,
    'views': 24004842608.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 12800000.0,
    'views': 9877365274.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3200000.0,
    'views': 14631710289.0,
    'category': 'Gaming',
    'population': 18952038.0,
    'Unemployment_rate': 7.09,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.5
  }, {
    'income': 7000000.0,
    'views': 13626331061.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 14700000.0,
    'views': 30788679536.0,
    'category': 'Entertainment',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 11200000.0,
    'views': 22519705183.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 15100000.0,
    'views': 27286058807.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1600000.0,
    'views': 10323391593.0,
    'category': 'Gaming',
    'population': 6453553.0,
    'Unemployment_rate': 4.11,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 29.4
  }, {
    'income': 11200000.0,
    'views': 21388725229.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6300000.0,
    'views': 16602198273.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2300000.0,
    'views': 7828610828.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 64200000.0,
    'views': 41139050371.0,
    'category': 'Entertainment',
    'population': 216565318.0,
    'Unemployment_rate': 4.45,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 9.0
  }, {
    'income': 6400000.0,
    'views': 25458952022.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1300000.0,
    'views': 4274709210.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 8500000.0,
    'views': 50292540392.0,
    'category': 'People & Blogs',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 17000000.0,
    'views': 27568757295.0,
    'category': 'People & Blogs',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 14600000.0,
    'views': 36458726976.0,
    'category': 'Film & Animation',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 55200000.0,
    'views': 37939780685.0,
    'category': 'Entertainment',
    'population': 216565318.0,
    'Unemployment_rate': 4.45,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 9.0
  }, {
    'income': 7700000.0,
    'views': 26820902622.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 32200000.0,
    'views': 24519022988.0,
    'category': 'Shows',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2600000.0,
    'views': 19547696190.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 8800000.0,
    'views': 22477745835.0,
    'category': 'Entertainment',
    'population': 287025.0,
    'Unemployment_rate': 10.33,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 65.4
  }, {
    'income': 8700000.0,
    'views': 17608931161.0,
    'category': 'Trailers',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6600000.0,
    'views': 5603111948.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 4700000.0,
    'views': 39450824833.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11900000.0,
    'views': 23884824160.0,
    'category': 'People & Blogs',
    'population': 9770529.0,
    'Unemployment_rate': 2.35,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 36.8
  }, {
    'income': 3800000.0,
    'views': 3294013141.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2800000.0,
    'views': 10507474316.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 26900000.0,
    'views': 44900897958.0,
    'category': 'Education',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 12700000.0,
    'views': 16118181673.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 6300000.0,
    'views': 36131228583.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 78500000.0,
    'views': 25154232306.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2200000.0,
    'views': 7339333120.0,
    'category': 'Nonprofits & Activism',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 76300.0,
    'views': 21528116909.0,
    'category': 'People & Blogs',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 39100000.0,
    'views': 16718192386.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1000000.0,
    'views': 22756581750.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 20000000.0,
    'views': 28519339489.0,
    'category': 'Shows',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 9900000.0,
    'views': 22731415608.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 15400000.0,
    'views': 18208196857.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6800000.0,
    'views': 27262462114.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 2400000.0,
    'views': 16402066717.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 12800000.0,
    'views': 13102611515.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 12100000.0,
    'views': 24188861917.0,
    'category': 'People & Blogs',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 8400000.0,
    'views': 19378155425.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7300000.0,
    'views': 22553923546.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1000000.0,
    'views': 3010784935.0,
    'category': 'News & Politics',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 14500000.0,
    'views': 27118354077.0,
    'category': 'Music',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 25600000.0,
    'views': 15657673422.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 14000000.0,
    'views': 16105023749.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 15900000.0,
    'views': 22637783517.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6100000.0,
    'views': 20297931219.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11500000.0,
    'views': 55299840198.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 16100000.0,
    'views': 25607397308.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4900000.0,
    'views': 12746535822.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Une=mployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6200000.0,
    'views': 23005313609.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 26300000.0,
    'views': 3963007415.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 8600000.0,
    'views': 11351015824.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1900000.0,
    'views': 15432929204.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 19400000.0,
    'views': 27274550757.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 10100000.0,
    'views': 23492684419.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5600000.0,
    'views': 13013567335.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 13100000.0,
    'views': 14864294792.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1200000.0,
    'views': 11405809704.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2700000.0,
    'views': 10530729078.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 6500000.0,
    'views': 20269857567.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7500000.0,
    'views': 5994136760.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 6100000.0,
    'views': 26355088167.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 972600.0,
    'views': 17853798780.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 15800000.0,
    'views': 28516250629.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 17900000.0,
    'views': 23379969006.0,
    'category': 'Music',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 22400000.0,
    'views': 18699145555.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 2700000.0,
    'views': 10602236110.0,
    'category': 'Shows',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2100000.0,
    'views': 13061739758.0,
    'category': 'Gaming',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 80000000.0,
    'views': 26800674545.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 38300000.0,
    'views': 19428308461.0,
    'category': 'Entertainment',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 1500000.0,
    'views': 6762424690.0,
    'category': 'Howto & Style',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 5200000.0,
    'views': 5711208484.0,
    'category': 'Science & Technology',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 5400000.0,
    'views': 16476978876.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7400000.0,
    'views': 21031745531.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1300000.0,
    'views': 2930015381.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 15000000.0,
    'views': 11615848291.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5000000.0,
    'views': 26583873105.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11800000.0,
    'views': 22919271731.0,
    'category': 'Film & Animation',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 9200000.0,
    'views': 6187804950.0,
    'category': 'Entertainment',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 9100000.0,
    'views': 16793072362.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1500000.0,
    'views': 3145161634.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 22200000.0,
    'views': 16709857823.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7600000.0,
    'views': 4521573939.0,
    'category': 'Gaming',
    'population': 4207083.0,
    'Unemployment_rate': 2.18,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 54.4
  }, {
    'income': 1000000.0,
    'views': 4332274962.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 2200000.0,
    'views': 14037426379.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 13546549817.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6600000.0,
    'views': 14199108016.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 5600000.0,
    'views': 27684955537.0,
    'category': 'Entertainment',
    'population': 10101694.0,
    'Unemployment_rate': 14.72,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 34.4
  }, {
    'income': 4800000.0,
    'views': 19607009165.0,
    'category': 'Music',
    'population': 17332850.0,
    'Unemployment_rate': 3.2,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 85.0
  }, {
    'income': 5200000.0,
    'views': 22593193994.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4300000.0,
    'views': 12831200855.0,
    'category': 'Music',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 1300000.0,
    'views': 4457913639.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3900000.0,
    'views': 17208027242.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 14500000.0,
    'views': 11627437847.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 582900.0,
    'views': 4079141673.0,
    'category': 'Comedy',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 5300000.0,
    'views': 17930570614.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1300000.0,
    'views': 9956764048.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 10900000.0,
    'views': 10062770060.0,
    'category': 'Movies',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 13800000.0,
    'views': 14814192034.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 32900000.0,
    'views': 23844936965.0,
    'category': 'Entertainment',
    'population': 216565318.0,
    'Unemployment_rate': 4.45,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 9.0
  }, {
    'income': 8900000.0,
    'views': 7600740993.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7600000.0,
    'views': 14412474625.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6100000.0,
    'views': 21037851468.0,
    'category': 'Music',
    'population': 25766605.0,
    'Unemployment_rate': 5.27,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 113.1
  }, {
    'income': 8300000.0,
    'views': 15777682516.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 4300000.0,
    'views': 13379395501.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 14800000.0,
    'views': 19883150017.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 8400000.0,
    'views': 10336420490.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3800000.0,
    'views': 19417887510.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2000000.0,
    'views': 22440611155.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4900000.0,
    'views': 7705492350.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 20600000.0,
    'views': 17318452893.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6900000.0,
    'views': 6570935979.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4000000.0,
    'views': 7938616641.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 13300000.0,
    'views': 4388047013.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1200000.0,
    'views': 10317306313.0,
    'category': 'Comedy',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 5700000.0,
    'views': 7173668905.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7800000.0,
    'views': 15065753455.0,
    'category': 'News & Politics',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 4300000.0,
    'views': 20358117330.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4800000.0,
    'views': 8595760553.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 50200000.0,
    'views': 27006526665.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 58800000.0,
    'views': 17211600007.0,
    'category': 'Entertainment',
    'population': 60297396.0,
    'Unemployment_rate': 9.89,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 61.9
  }, {
    'income': 828600.0,
    'views': 4749833967.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 34600000.0,
    'views': 16097531087.0,
    'category': 'Entertainment',
    'population': 126226568.0,
    'Unemployment_rate': 2.29,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 63.2
  }, {
    'income': 3300000.0,
    'views': 10435474336.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2700000.0,
    'views': 17308961985.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11000000.0,
    'views': 11372071889.0,
    'category': 'Music',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 2600000.0,
    'views': 15541421838.0,
    'category': 'Gaming',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 7900000.0,
    'views': 17793809548.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1500000.0,
    'views': 7466926260.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 5000000.0,
    'views': 10242981063.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 15300000.0,
    'views': 25592378292.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 9400000.0,
    'views': 14401218086.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2800000.0,
    'views': 6430853035.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 39100000.0,
    'views': 34300482066.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 9500000.0,
    'views': 17331663193.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 6700000.0,
    'views': 11081602368.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 13100000.0,
    'views': 10409352249.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4500000.0,
    'views': 19458807708.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2400000.0,
    'views': 16357064198.0,
    'category': 'Music',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 5900000.0,
    'views': 14169516119.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 13600000.0,
    'views': 14827085149.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 15900000.0,
    'views': 17387583720.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 16100.0,
    'views': 2588501115.0,
    'category': 'Howto & Style',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 6500000.0,
    'views': 3699352704.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6700000.0,
    'views': 20531704527.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 2994726412.0,
    'category': 'Howto & Style',
    'population': 25766605.0,
    'Unemployment_rate': 5.27,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 113.1
  }, {
    'income': 4300000.0,
    'views': 23755792542.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4300000.0,
    'views': 3647987299.0,
    'category': 'Science & Technology',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5100000.0,
    'views': 23962070944.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 8200000.0,
    'views': 12385924995.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4600000.0,
    'views': 6608773195.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1600000.0,
    'views': 2700914170.0,
    'category': 'Entertainment',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 19300000.0,
    'views': 10999000479.0,
    'category': 'Comedy',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 20400000.0,
    'views': 56106087508.0,
    'category': 'Music',
    'population': 83429615.0,
    'Unemployment_rate': 13.49,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 23.9
  }, {
    'income': 3000000.0,
    'views': 8425505919.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 261100.0,
    'views': 6002166932.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 36500000.0,
    'views': 13943030228.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 7300000.0,
    'views': 8279004442.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2800000.0,
    'views': 4067878931.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 520900.0,
    'views': 6582932625.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 8500000.0,
    'views': 7213499085.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4700000.0,
    'views': 10414479943.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5900000.0,
    'views': 17688774915.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 401700.0,
    'views': 2543809954.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1600000.0,
    'views': 15510153803.0,
    'category': 'People & Blogs',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 36200000.0,
    'views': 20289689389.0,
    'category': 'Pets & Animals',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2200000.0,
    'views': 2135644776.0,
    'category': 'Entertainment',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 518600.0,
    'views': 5994002464.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 6766461070.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2600000.0,
    'views': 7920637200.0,
    'category': 'Film & Animation',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2000000.0,
    'views': 14777034543.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2900000.0,
    'views': 14696994366.0,
    'category': 'People & Blogs',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 17600000.0,
    'views': 9465863821.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1400000.0,
    'views': 7926899136.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 6000000.0,
    'views': 22471357411.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6900000.0,
    'views': 15751661213.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2500000.0,
    'views': 12889240875.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6300000.0,
    'views': 25579831081.0,
    'category': 'Music',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 635800.0,
    'views': 2551113422.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 14600000.0,
    'views': 9299371231.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6200000.0,
    'views': 13151870846.0,
    'category': 'Shows',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 36300000.0,
    'views': 31494513067.0,
    'category': 'Entertainment',
    'population': 83132799.0,
    'Unemployment_rate': 3.04,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 70.2
  }, {
    'income': 1100000.0,
    'views': 3378047383.0,
    'category': 'Science & Technology',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1700000.0,
    'views': 6041264489.0,
    'category': 'Entertainment',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 4500000.0,
    'views': 10939966484.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8100000.0,
    'views': 16298342829.0,
    'category': 'Music',
    'population': 10101694.0,
    'Unemployment_rate': 14.72,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 34.4
  }, {
    'income': 6600000.0,
    'views': 13206471140.0,
    'category': 'Education',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 7500000.0,
    'views': 10115316784.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 47100000.0,
    'views': 20847038152.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 19200000.0,
    'views': 9223534599.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3900000.0,
    'views': 14231943358.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3500000.0,
    'views': 17507060680.0,
    'category': 'People & Blogs',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 4600000.0,
    'views': 13405849040.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 56300000.0,
    'views': 27084848152.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2300000.0,
    'views': 2431154438.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8400000.0,
    'views': 13835173331.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3500000.0,
    'views': 8040036209.0,
    'category': 'Music',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 6300000.0,
    'views': 24059336857.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6700000.0,
    'views': 11136266461.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5400000.0,
    'views': 9924807127.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 34100000.0,
    'views': 18347969186.0,
    'category': 'Entertainment',
    'population': 126226568.0,
    'Unemployment_rate': 2.29,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 63.2
  }, {
    'income': 8900000.0,
    'views': 4454917643.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11200000.0,
    'views': 5918314128.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 33000000.0,
    'views': 15552070846.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 9500000.0,
    'views': 4469711607.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2200000.0,
    'views': 5614621131.0,
    'category': 'Science & Technology',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6100000.0,
    'views': 11288359365.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2500000.0,
    'views': 12140232004.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 9300000.0,
    'views': 9597894786.0,
    'category': 'Autos & Vehicles',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 9300000.0,
    'views': 5863377051.0,
    'category': 'Howto & Style',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3100000.0,
    'views': 9346383505.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5400000.0,
    'views': 5890180734.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7700000.0,
    'views': 6264261757.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7700000.0,
    'views': 6269945014.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5000000.0,
    'views': 10644857969.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 35000000.0,
    'views': 12761253839.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3300000.0,
    'views': 5141834668.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11300000.0,
    'views': 10047736580.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 26900000.0,
    'views': 12895427184.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6800000.0,
    'views': 11364908616.0,
    'category': 'Entertainment',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 1200000.0,
    'views': 5783557209.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1300000.0,
    'views': 4526271677.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 118900.0,
    'views': 4380564906.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5700000.0,
    'views': 7762905663.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 3800000.0,
    'views': 10631638628.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1400000.0,
    'views': 1693149479.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2500000.0,
    'views': 3828000587.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 18400000.0,
    'views': 11058049885.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 1870608170.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1400000.0,
    'views': 2378448129.0,
    'category': 'Education',
    'population': 83132799.0,
    'Unemployment_rate': 3.04,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 70.2
  }, {
    'income': 42000000.0,
    'views': 17963202261.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 960600.0,
    'views': 5091618606.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 26500000.0,
    'views': 8658941531.0,
    'category': 'People & Blogs',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 3500000.0,
    'views': 12624879732.0,
    'category': 'Sports',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 9300000.0,
    'views': 7657171980.0,
    'category': 'Comedy',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 3900000.0,
    'views': 10292874715.0,
    'category': 'Music',
    'population': 10285453.0,
    'Unemployment_rate': 6.48,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 67.0
  }, {
    'income': 7600000.0,
    'views': 15038593883.0,
    'category': 'Entertainment',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 1300000.0,
    'views': 8768697257.0,
    'category': 'Gaming',
    'population': 25766605.0,
    'Unemployment_rate': 5.27,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 113.1
  }, {
    'income': 9400000.0,
    'views': 11009148579.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1900000.0,
    'views': 1796227417.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 703000.0,
    'views': 7311322368.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 43600000.0,
    'views': 29406206620.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1400000.0,
    'views': 4707412332.0,
    'category': 'Entertainment',
    'population': 44385155.0,
    'Unemployment_rate': 8.88,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 82.7
  }, {
    'income': 2000000.0,
    'views': 3579555124.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4100000.0,
    'views': 2441288701.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 5400000.0,
    'views': 11819051552.0,
    'category': 'Education',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 12700000.0,
    'views': 2951914200.0,
    'category': 'Entertainment',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 746700.0,
    'views': 2764127969.0,
    'category': 'Comedy',
    'population': 9770529.0,
    'Unemployment_rate': 2.35,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 36.8
  }, {
    'income': 5400000.0,
    'views': 7274150246.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 59800000.0,
    'views': 20919403720.0,
    'category': 'Comedy',
    'population': 1912789.0,
    'Unemployment_rate': 6.52,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.1
  }, {
    'income': 21700000.0,
    'views': 19694265358.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3100000.0,
    'views': 6098644584.0,
    'category': 'Gaming',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 10600000.0,
    'views': 23353115850.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1200000.0,
    'views': 6618524158.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 260200.0,
    'views': 8920141342.0,
    'category': 'Gaming',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 713400.0,
    'views': 11317309935.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 13300000.0,
    'views': 14816075927.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5100000.0,
    'views': 6119294270.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 437400.0,
    'views': 17913237851.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 13154314376.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 63900.0,
    'views': 559765455.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1900000.0,
    'views': 3875172235.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 10300000.0,
    'views': 13917423958.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 271200.0,
    'views': 3234880084.0,
    'category': 'Science & Technology',
    'population': 83132799.0,
    'Unemployment_rate': 3.04,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 70.2
  }, {
    'income': 294600.0,
    'views': 5759442450.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 180900.0,
    'views': 1803249241.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2600000.0,
    'views': 11323617496.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 13200000.0,
    'views': 13930021471.0,
    'category': 'Shows',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5300000.0,
    'views': 8779729549.0,
    'category': 'Entertainment',
    'population': 83429615.0,
    'Unemployment_rate': 13.49,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 23.9
  }, {
    'income': 110600000.0,
    'views': 7906181776.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 3961318438.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1500000.0,
    'views': 2255542592.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 114300.0,
    'views': 1577859332.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 13100000.0,
    'views': 23038014291.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2800000.0,
    'views': 5529131886.0,
    'category': 'Sports',
    'population': 8574832.0,
    'Unemployment_rate': 4.58,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 59.6
  }, {
    'income': 131100.0,
    'views': 4329121104.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1400000.0,
    'views': 7590582024.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 827600.0,
    'views': 13124645973.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1500000.0,
    'views': 4566120190.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1000000.0,
    'views': 4924054368.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5900000.0,
    'views': 7229175322.0,
    'category': 'Howto & Style',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 5100000.0,
    'views': 8281724393.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3700000.0,
    'views': 13824277846.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 4900000.0,
    'views': 16014044618.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 5200000.0,
    'views': 15126092508.0,
    'category': 'Entertainment',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 1200000.0,
    'views': 2855519150.0,
    'category': 'People & Blogs',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 6700000.0,
    'views': 9813245108.0,
    'category': 'Education',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 1400000.0,
    'views': 3654621568.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3800000.0,
    'views': 5257834105.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 4700000.0,
    'views': 9573641299.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3300000.0,
    'views': 9594188708.0,
    'category': 'Entertainment',
    'population': 44385155.0,
    'Unemployment_rate': 8.88,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 82.7
  }, {
    'income': 5200000.0,
    'views': 3276891538.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4900000.0,
    'views': 6047584292.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8100000.0,
    'views': 20196704276.0,
    'category': 'Music',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 2100000.0,
    'views': 8761255550.0,
    'category': 'Gaming',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 1800000.0,
    'views': 3457618361.0,
    'category': 'Gaming',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 2300000.0,
    'views': 8147575884.0,
    'category': 'People & Blogs',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 9500000.0,
    'views': 4051072188.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 692600.0,
    'views': 4120324310.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 528200.0,
    'views': 7038827526.0,
    'category': 'Comedy',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 2600000.0,
    'views': 11544297793.0,
    'category': 'Music',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 12800000.0,
    'views': 25367126292.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 819000.0,
    'views': 1606834186.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 6100000.0,
    'views': 7760819588.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6700000.0,
    'views': 1556003039.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 866200.0,
    'views': 3213324455.0,
    'category': 'People & Blogs',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 23900000.0,
    'views': 13378360425.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1300000.0,
    'views': 3306242674.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 2300000.0,
    'views': 9983065083.0,
    'category': 'Gaming',
    'population': 67059887.0,
    'Unemployment_rate': 8.43,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 65.6
  }, {
    'income': 8700000.0,
    'views': 14857290259.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3100000.0,
    'views': 10703830496.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 2600000.0,
    'views': 3980991248.0,
    'category': 'Howto & Style',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 18800000.0,
    'views': 9601137077.0,
    'category': 'Sports',
    'population': 25766605.0,
    'Unemployment_rate': 5.27,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 113.1
  }, {
    'income': 5300000.0,
    'views': 10463166404.0,
    'category': 'News & Politics',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 12400000.0,
    'views': 17921124985.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 6404852037.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 415200.0,
    'views': 7176572299.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2500000.0,
    'views': 3841205465.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1300000.0,
    'views': 5168721499.0,
    'category': 'Entertainment',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 890300.0,
    'views': 5244917119.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1400000.0,
    'views': 6888074944.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4100000.0,
    'views': 9867515979.0,
    'category': 'Entertainment',
    'population': 96462106.0,
    'Unemployment_rate': 2.01,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.5
  }, {
    'income': 21500000.0,
    'views': 9887116267.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4900000.0,
    'views': 16174530046.0,
    'category': 'Shows',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 20700000.0,
    'views': 11057945183.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4700000.0,
    'views': 20115544708.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 16600000.0,
    'views': 7912733203.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2300000.0,
    'views': 3647267655.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 1700000.0,
    'views': 7739048000.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 17800000.0,
    'views': 8396875537.0,
    'category': 'News & Politics',
    'population': 44385155.0,
    'Unemployment_rate': 8.88,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 82.7
  }, {
    'income': 5700000.0,
    'views': 9059696049.0,
    'category': 'Film & Animation',
    'population': 32447385.0,
    'Unemployment_rate': 3.32,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 45.1
  }, {
    'income': 30700000.0,
    'views': 17247584185.0,
    'category': 'Film & Animation',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 2600000.0,
    'views': 9269174070.0,
    'category': 'Gaming',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 1300000.0,
    'views': 2274007523.0,
    'category': 'Comedy',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 1100000.0,
    'views': 2977741577.0,
    'category': 'Howto & Style',
    'population': 1397715000.0,
    'Unemployment_rate': 4.32,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 50.6
  }, {
    'income': 4800000.0,
    'views': 6306204566.0,
    'category': 'Gaming',
    'population': 18952038.0,
    'Unemployment_rate': 7.09,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.5
  }, {
    'income': 532700.0,
    'views': 3802280098.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 1000000.0,
    'views': 2238134438.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 18900000.0,
    'views': 7263619576.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 5300000.0,
    'views': 7612385622.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4700000.0,
    'views': 16107116549.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 21100000.0,
    'views': 17763586483.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11300000.0,
    'views': 13043561912.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 5400000.0,
    'views': 11144195464.0,
    'category': 'Music',
    'population': 39309783.0,
    'Unemployment_rate': 12.82,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 16.2
  }, {
    'income': 2200000.0,
    'views': 1026425106.0,
    'category': 'Science & Technology',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 40200.0,
    'views': 902225615.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 15900000.0,
    'views': 11371738047.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 745200.0,
    'views': 3684816159.0,
    'category': 'Howto & Style',
    'population': 25766605.0,
    'Unemployment_rate': 5.27,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 113.1
  }, {
    'income': 2500000.0,
    'views': 3606912471.0,
    'category': 'Science & Technology',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3500000.0,
    'views': 7337212581.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 4000000.0,
    'views': 11445492404.0,
    'category': 'Education',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 739500.0,
    'views': 5024088947.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 1800000.0,
    'views': 4448334716.0,
    'category': 'Comedy',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 2200000.0,
    'views': 9710962528.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 84500.0,
    'views': 2173106162.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4500000.0,
    'views': 10847948832.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 366100.0,
    'views': 1001465469.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2400000.0,
    'views': 14543594822.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3500000.0,
    'views': 8684010451.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 522600.0,
    'views': 9111000228.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 78500.0,
    'views': 3827906874.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1700000.0,
    'views': 4609753237.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4300000.0,
    'views': 9270331567.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 13200000.0,
    'views': 9544277833.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7200000.0,
    'views': 3523578665.0,
    'category': 'Nonprofits & Activism',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2100000.0,
    'views': 7195314800.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 2100000.0,
    'views': 3693798804.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 5000000.0,
    'views': 7102965621.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2200000.0,
    'views': 8826138204.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 774700.0,
    'views': 5089284369.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 2800000.0,
    'views': 7435180827.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7400000.0,
    'views': 9439857193.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4300000.0,
    'views': 11946217860.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 29600000.0,
    'views': 15278668857.0,
    'category': 'Entertainment',
    'population': 83132799.0,
    'Unemployment_rate': 3.04,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 70.2
  }, {
    'income': 9600000.0,
    'views': 5819508534.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 2440934034.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5400000.0,
    'views': 3037260680.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6300000.0,
    'views': 7043235131.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1200000.0,
    'views': 7406207930.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 865300.0,
    'views': 3955426159.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 448400.0,
    'views': 4434679706.0,
    'category': 'Comedy',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 6400000.0,
    'views': 7135820721.0,
    'category': 'Sports',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4200000.0,
    'views': 9463244435.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 8700000.0,
    'views': 12844432341.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 1400000.0,
    'views': 6578828147.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2000000.0,
    'views': 6613422635.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1900000.0,
    'views': 7520242626.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 4100000.0,
    'views': 10170264839.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 4200000.0,
    'views': 12475714382.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7300000.0,
    'views': 7141825267.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 861300.0,
    'views': 1656452554.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 147800.0,
    'views': 4768370464.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 3000000.0,
    'views': 10227242833.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1200000.0,
    'views': 8091706232.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1600000.0,
    'views': 2990185467.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1300000.0,
    'views': 4076692623.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 12300000.0,
    'views': 9763592867.0,
    'category': 'Music',
    'population': 96462106.0,
    'Unemployment_rate': 2.01,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.5
  }, {
    'income': 10800000.0,
    'views': 14563841315.0,
    'category': 'News & Politics',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 5600000.0,
    'views': 20522339099.0,
    'category': 'Entertainment',
    'population': 5703569.0,
    'Unemployment_rate': 4.11,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 84.8
  }, {
    'income': 461700.0,
    'views': 1517638132.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 662500.0,
    'views': 2687443643.0,
    'category': 'People & Blogs',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 19200000.0,
    'views': 6872702790.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 12800000.0,
    'views': 15812009296.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 685800.0,
    'views': 3018032423.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 225500.0,
    'views': 3029872908.0,
    'category': 'Gaming',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 5400000.0,
    'views': 5997599089.0,
    'category': 'Film & Animation',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 475900.0,
    'views': 9962188084.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 96300.0,
    'views': 2609358468.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3100000.0,
    'views': 6534326412.0,
    'category': 'Gaming',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 6500000.0,
    'views': 9198986881.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 7900000.0,
    'views': 9052367553.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2900000.0,
    'views': 6802382479.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 1200000.0,
    'views': 1349288771.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8900000.0,
    'views': 3392918989.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3400000.0,
    'views': 4122634467.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 1200000.0,
    'views': 13116313599.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2500000.0,
    'views': 9787697838.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6800000.0,
    'views': 5914071870.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1500000.0,
    'views': 6626563508.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4100000.0,
    'views': 5558051295.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 16100000.0,
    'views': 8696631898.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4200000.0,
    'views': 7172386509.0,
    'category': 'Science & Technology',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 2700000.0,
    'views': 3869457097.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1300000.0,
    'views': 6165757696.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4300000.0,
    'views': 8984089026.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 4400000.0,
    'views': 6386271870.0,
    'category': 'Music',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 8100000.0,
    'views': 7776706184.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 6500000.0,
    'views': 8265129639.0,
    'category': 'Shows',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 7900000.0,
    'views': 14619523361.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 6700000.0,
    'views': 12714141740.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1300000.0,
    'views': 6396049701.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1600000.0,
    'views': 4499826716.0,
    'category': 'Howto & Style',
    'population': 100388073.0,
    'Unemployment_rate': 10.76,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 35.2
  }, {
    'income': 1300000.0,
    'views': 1781226000.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3800000.0,
    'views': 11513738907.0,
    'category': 'Gaming',
    'population': 9770529.0,
    'Unemployment_rate': 2.35,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 36.8
  }, {
    'income': 2700000.0,
    'views': 1543608082.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 76000.0,
    'views': 5469103954.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 8000000.0,
    'views': 3736069980.0,
    'category': 'Science & Technology',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 4000000.0,
    'views': 14564170905.0,
    'category': 'Music',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 1800000.0,
    'views': 4909687948.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 947500.0,
    'views': 4750420071.0,
    'category': 'People & Blogs',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 4100000.0,
    'views': 2656528205.0,
    'category': 'Sports',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 3700000.0,
    'views': 10714145606.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 284200.0,
    'views': 1640078055.0,
    'category': 'Gaming',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 22800000.0,
    'views': 9938811455.0,
    'category': 'People & Blogs',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 17400000.0,
    'views': 16545966132.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3100000.0,
    'views': 857725714.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4700000.0,
    'views': 4503880875.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 479000.0,
    'views': 1491452935.0,
    'category': 'Entertainment',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 11.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 2800000.0,
    'views': 6391679636.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 10100000.0,
    'views': 7151683497.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 5100000.0,
    'views': 7564652648.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 212500.0,
    'views': 8333387785.0,
    'category': 'Entertainment',
    'population': 83429615.0,
    'Unemployment_rate': 13.49,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 23.9
  }, {
    'income': 9000000.0,
    'views': 14198154095.0,
    'category': 'News & Politics',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2400000.0,
    'views': 6624168155.0,
    'category': 'News & Politics',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 767200.0,
    'views': 1777072487.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 827100.0,
    'views': 2143140898.0,
    'category': 'Education',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 594000.0,
    'views': 1760131384.0,
    'category': 'People & Blogs',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 2100000.0,
    'views': 3060202847.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 573900.0,
    'views': 2761547758.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 286600.0,
    'views': 4236036141.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 2200000.0,
    'views': 5324913850.0,
    'category': 'People & Blogs',
    'population': 100388073.0,
    'Unemployment_rate': 10.76,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 35.2
  }, {
    'income': 4600000.0,
    'views': 6668507856.0,
    'category': 'Gaming',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 2000000.0,
    'views': 7857371770.0,
    'category': 'Music',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 24600000.0,
    'views': 9477021288.0,
    'category': 'Comedy',
    'population': 44938712.0,
    'Unemployment_rate': 9.79,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 90.0
  }, {
    'income': 8700000.0,
    'views': 10489367372.0,
    'category': 'News & Politics',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 34200000.0,
    'views': 13897932103.0,
    'category': 'Entertainment',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 4100000.0,
    'views': 12413869881.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 12000000.0,
    'views': 4352427049.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 5500000.0,
    'views': 9924103188.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 7000000.0,
    'views': 8897705695.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2100000.0,
    'views': 7159750970.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1600000.0,
    'views': 4008801873.0,
    'category': 'Entertainment',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 2000000.0,
    'views': 4741434420.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1400000.0,
    'views': 6889304306.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4900000.0,
    'views': 8658553456.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 11600000.0,
    'views': 9978734160.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 15000000.0,
    'views': 10239836582.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5400000.0,
    'views': 11827310821.0,
    'category': 'News & Politics',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 13600000.0,
    'views': 13562853889.0,
    'category': 'Comedy',
    'population': 126226568.0,
    'Unemployment_rate': 2.29,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 63.2
  }, {
    'income': 8800000.0,
    'views': 11506702632.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 6300000.0,
    'views': 8932038210.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 5800000.0,
    'views': 10069000444.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 8074260978.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 530100.0,
    'views': 4033400167.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2700000.0,
    'views': 4395184343.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 665200.0,
    'views': 4977284539.0,
    'category': 'Entertainment',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 2400000.0,
    'views': 5549770244.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 1900000.0,
    'views': 5956193599.0,
    'category': 'Gaming',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2900000.0,
    'views': 13356517783.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 344500.0,
    'views': 1833519700.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1100000.0,
    'views': 3587576784.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 31.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1000000.0,
    'views': 5269059435.0,
    'category': 'Gaming',
    'population': 25766605.0,
    'Unemployment_rate': 5.27,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 113.1
  }, {
    'income': 2300000.0,
    'views': 7018015062.0,
    'category': 'Howto & Style',
    'population': 44385155.0,
    'Unemployment_rate': 8.88,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 82.7
  }, {
    'income': 2500000.0,
    'views': 8649303688.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4200000.0,
    'views': 9076642765.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2600000.0,
    'views': 8866012877.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 129300.0,
    'views': 2230986039.0,
    'category': 'Entertainment',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 229500.0,
    'views': 1321380490.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 10900.0,
    'views': 1506796393.0,
    'category': 'Entertainment',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 911000.0,
    'views': 2465473772.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3600000.0,
    'views': 4029253667.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1700000.0,
    'views': 6751985988.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3800000.0,
    'views': 8882319696.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 16700000.0,
    'views': 12362331529.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8900000.0,
    'views': 12961669452.0,
    'category': 'News & Politics',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2000000.0,
    'views': 4684983333.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 787600.0,
    'views': 2613197447.0,
    'category': 'Entertainment',
    'population': 17373662.0,
    'Unemployment_rate': 3.97,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 44.9
  }, {
    'income': 2900000.0,
    'views': 3337074920.0,
    'category': 'People & Blogs',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 4400000.0,
    'views': 3603556207.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 506300.0,
    'views': 4622581344.0,
    'category': 'People & Blogs',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 7300000.0,
    'views': 5766647017.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1700000.0,
    'views': 6017932195.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5300000.0,
    'views': 11182302317.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1800000.0,
    'views': 7952268926.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2400000.0,
    'views': 2750902766.0,
    'category': 'Howto & Style',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 6700000.0,
    'views': 5525773746.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1400000.0,
    'views': 6944967581.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 21600.0,
    'views': 4577292740.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 136200.0,
    'views': 3551889957.0,
    'category': 'Howto & Style',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 475700.0,
    'views': 4260187928.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3200000.0,
    'views': 4598387043.0,
    'category': 'News & Politics',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 238100.0,
    'views': 5014888374.0,
    'category': 'People & Blogs',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 2300000.0,
    'views': 6290721701.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2300000.0,
    'views': 8582696157.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2200000.0,
    'views': 9383692066.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 20100000.0,
    'views': 10303519926.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4700000.0,
    'views': 20042571499.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1800000.0,
    'views': 4315486422.0,
    'category': 'Music',
    'population': 36910560.0,
    'Unemployment_rate': 9.02,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 35.9
  }, {
    'income': 255800.0,
    'views': 3517662420.0,
    'category': 'Comedy',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 65700.0,
    'views': 600154268.0,
    'category': 'Entertainment',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 666800.0,
    'views': 2224121890.0,
    'category': 'Gaming',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 48100.0,
    'views': 1629801448.0,
    'category': 'Entertainment',
    'population': 32510453.0,
    'Unemployment_rate': 3.31,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 70.7
  }, {
    'income': 224300.0,
    'views': 2972474215.0,
    'category': 'Entertainment',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 548000.0,
    'views': 3086254545.0,
    'category': 'Comedy',
    'population': 67059887.0,
    'Unemployment_rate': 8.43,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 65.6
  }, {
    'income': 414500.0,
    'views': 3900312631.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 170700.0,
    'views': 4035738731.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3400000.0,
    'views': 4156427797.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1300000.0,
    'views': 4597228794.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 10200000.0,
    'views': 8011977288.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4300000.0,
    'views': 11423792969.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 7200000.0,
    'views': 18515587421.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 19900.0,
    'views': 4776507159.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3000000.0,
    'views': 6048517979.0,
    'category': 'Music',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 913400.0,
    'views': 4040297006.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1700000.0,
    'views': 1778318927.0,
    'category': 'Howto & Style',
    'population': 17332850.0,
    'Unemployment_rate': 3.2,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 85.0
  }, {
    'income': 2600000.0,
    'views': 2084791147.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2100000.0,
    'views': 3920559552.0,
    'category': 'Gaming',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 6800000.0,
    'views': 6554000320.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 17700000.0,
    'views': 10238593147.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2900000.0,
    'views': 11428794827.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 619300.0,
    'views': 3317805543.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1200000.0,
    'views': 2131548711.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 17000000.0,
    'views': 5405563355.0,
    'category': 'Autos & Vehicles',
    'population': 9770529.0,
    'Unemployment_rate': 2.35,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 36.8
  }, {
    'income': 8500000.0,
    'views': 6036496916.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 6500000.0,
    'views': 6884215292.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4700000.0,
    'views': 19013942981.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 490600.0,
    'views': 3280481927.0,
    'category': 'Sports',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 600100.0,
    'views': 4627069704.0,
    'category': 'Entertainment',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 490100.0,
    'views': 4959982720.0,
    'category': 'Gaming',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 658000.0,
    'views': 2214167846.0,
    'category': 'Howto & Style',
    'population': 10285453.0,
    'Unemployment_rate': 6.48,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 67.0
  }, {
    'income': 1300000.0,
    'views': 4674164601.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 7200000.0,
    'views': 7719743112.0,
    'category': 'Gaming',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 9700000.0,
    'views': 8623705301.0,
    'category': 'Music',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 3400000.0,
    'views': 9660950823.0,
    'category': 'Music',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 65500000.0,
    'views': 18917687143.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 6400000.0,
    'views': 13542939513.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3300000.0,
    'views': 5094050461.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 5100000.0,
    'views': 5673347763.0,
    'category': 'Gaming',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 4200000.0,
    'views': 9106781518.0,
    'category': 'Entertainment',
    'population': 10101694.0,
    'Unemployment_rate': 14.72,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 34.4
  }, {
    'income': 220700.0,
    'views': 2244318380.0,
    'category': 'Film & Animation',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1600000.0,
    'views': 5217553897.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1300000.0,
    'views': 5465532801.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4700000.0,
    'views': 12513842343.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 2165885634.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4800000.0,
    'views': 2224911030.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4000000.0,
    'views': 1820559912.0,
    'category': 'Science & Technology',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 2600000.0,
    'views': 2480957682.0,
    'category': 'Gaming',
    'population': 17332850.0,
    'Unemployment_rate': 3.2,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 85.0
  }, {
    'income': 1300000.0,
    'views': 5019136690.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 4800000.0,
    'views': 5727888539.0,
    'category': 'News & Politics',
    'population': 216565318.0,
    'Unemployment_rate': 4.45,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 9.0
  }, {
    'income': 3800000.0,
    'views': 6646953396.0,
    'category': 'Shows',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 10400000.0,
    'views': 11039343563.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4200000.0,
    'views': 1967930734.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1800000.0,
    'views': 1973638757.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 357000.0,
    'views': 1950178163.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 18200000.0,
    'views': 9596430464.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1100000.0,
    'views': 2939201386.0,
    'category': 'Howto & Style',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1300000.0,
    'views': 4963275018.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1700000.0,
    'views': 5457203710.0,
    'category': 'Comedy',
    'population': 96462106.0,
    'Unemployment_rate': 2.01,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.5
  }, {
    'income': 3100000.0,
    'views': 8134379376.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1100000.0,
    'views': 5178142148.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2300000.0,
    'views': 2122062016.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 1500000.0,
    'views': 1948925559.0,
    'category': 'Sports',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 733300.0,
    'views': 3764608356.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 516700.0,
    'views': 5141201173.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2100000.0,
    'views': 9685060624.0,
    'category': 'Pets & Animals',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 2700000.0,
    'views': 14717282742.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 644900.0,
    'views': 1181292450.0,
    'category': 'Gaming',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 1600000.0,
    'views': 4301581610.0,
    'category': 'Gaming',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 1400000.0,
    'views': 3445794123.0,
    'category': 'Howto & Style',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1700000.0,
    'views': 5545936485.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 3600000.0,
    'views': 8265920659.0,
    'category': 'Entertainment',
    'population': 44385155.0,
    'Unemployment_rate': 8.88,
    'created_date': 21.0,
    'Gross_tertiary_education_enrollment': 82.7
  }, {
    'income': 8600000.0,
    'views': 3912334359.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 23800000.0,
    'views': 11717217293.0,
    'category': 'Comedy',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 5300000.0,
    'views': 7958771872.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 517800.0,
    'views': 2750993392.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 64100.0,
    'views': 1900272833.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 11000000.0,
    'views': 11789678655.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 50100.0,
    'views': 2139769210.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 695900.0,
    'views': 2319515787.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2000000.0,
    'views': 3363634923.0,
    'category': 'Comedy',
    'population': 69625582.0,
    'Unemployment_rate': 0.75,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 49.3
  }, {
    'income': 805700.0,
    'views': 4356686216.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5500000.0,
    'views': 9569814790.0,
    'category': 'News & Politics',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 19100000.0,
    'views': 10022557589.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 2262690743.0,
    'category': 'Science & Technology',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 753300.0,
    'views': 6050102764.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 92600.0,
    'views': 3299216601.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 203200.0,
    'views': 3733856870.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2200000.0,
    'views': 4177184071.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4300000.0,
    'views': 6482687220.0,
    'category': 'People & Blogs',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 7800000.0,
    'views': 9088562002.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 9100000.0,
    'views': 7773543609.0,
    'category': 'Gaming',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 838500.0,
    'views': 7406628736.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 2831275503.0,
    'category': 'Howto & Style',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3400000.0,
    'views': 4129249415.0,
    'category': 'Gaming',
    'population': 34268528.0,
    'Unemployment_rate': 5.93,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 68.0
  }, {
    'income': 258600.0,
    'views': 6412313570.0,
    'category': 'Gaming',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 15300000.0,
    'views': 9884886099.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 420900.0,
    'views': 1148422000.0,
    'category': 'Entertainment',
    'population': 144373535.0,
    'Unemployment_rate': 4.59,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 81.9
  }, {
    'income': 977700.0,
    'views': 2036408398.0,
    'category': 'Gaming',
    'population': 5520314.0,
    'Unemployment_rate': 6.59,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1200000.0,
    'views': 3789736218.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 3100000.0,
    'views': 5224764969.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2500000.0,
    'views': 5263540904.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4100000.0,
    'views': 20743586601.0,
    'category': 'Music',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 5700000.0,
    'views': 9378175604.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 888900.0,
    'views': 1138262456.0,
    'category': 'Science & Technology',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 551500.0,
    'views': 1758603195.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 520700.0,
    'views': 1401914513.0,
    'category': 'People & Blogs',
    'population': 108116615.0,
    'Unemployment_rate': 2.15,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 35.5
  }, {
    'income': 1700000.0,
    'views': 4712624489.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2100000.0,
    'views': 2555801802.0,
    'category': 'People & Blogs',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1300000.0,
    'views': 4214172991.0,
    'category': 'Film & Animation',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2900000.0,
    'views': 4399833602.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2300000.0,
    'views': 4608751851.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 3000000.0,
    'views': 2879263916.0,
    'category': 'People & Blogs',
    'population': 17373662.0,
    'Unemployment_rate': 3.97,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 44.9
  }, {
    'income': 844200.0,
    'views': 5333569294.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2400000.0,
    'views': 5264039679.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1400000.0,
    'views': 2683297849.0,
    'category': 'Entertainment',
    'population': 60297396.0,
    'Unemployment_rate': 9.89,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 61.9
  }, {
    'income': 146500.0,
    'views': 1024467771.0,
    'category': 'Comedy',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1500000.0,
    'views': 1698279553.0,
    'category': 'Music',
    'population': 67059887.0,
    'Unemployment_rate': 8.43,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 65.6
  }, {
    'income': 51900000.0,
    'views': 8739174649.0,
    'category': 'Gaming',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 2000000.0,
    'views': 9999238237.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 17700000.0,
    'views': 6270909026.0,
    'category': 'Entertainment',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 1400000.0,
    'views': 4349562794.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 1500000.0,
    'views': 2112274210.0,
    'category': 'People & Blogs',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 575700.0,
    'views': 2509752944.0,
    'category': 'Sports',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 192100.0,
    'views': 3178222797.0,
    'category': 'Entertainment',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 1400000.0,
    'views': 3643698504.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 4400000.0,
    'views': 6300933122.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3900000.0,
    'views': 7520379951.0,
    'category': 'People & Blogs',
    'population': 9770529.0,
    'Unemployment_rate': 2.35,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 36.8
  }, {
    'income': 3600000.0,
    'views': 15446707595.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3400000.0,
    'views': 5585085130.0,
    'category': 'Shows',
    'population': 44385155.0,
    'Unemployment_rate': 8.88,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 82.7
  }, {
    'income': 40200.0,
    'views': 2848466522.0,
    'category': 'Education',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 1300000.0,
    'views': 3632438963.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 1700000.0,
    'views': 5863456698.0,
    'category': 'Comedy',
    'population': 39309783.0,
    'Unemployment_rate': 12.82,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 16.2
  }, {
    'income': 2600000.0,
    'views': 6662288136.0,
    'category': 'Music',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 869600.0,
    'views': 6970899521.0,
    'category': 'Entertainment',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 11000000.0,
    'views': 9502983550.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 10300000.0,
    'views': 7876740921.0,
    'category': 'Education',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 23.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 4300000.0,
    'views': 14185611472.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 2.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 3700000.0,
    'views': 4266957149.0,
    'category': 'Comedy',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 484000.0,
    'views': 4579773883.0,
    'category': 'Music',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 22700.0,
    'views': 9927699419.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 7.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 8200000.0,
    'views': 13174393401.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 4.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2100000.0,
    'views': 4733873025.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 173700.0,
    'views': 1081285962.0,
    'category': 'Gaming',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 15700000.0,
    'views': 1714955279.0,
    'category': 'People & Blogs',
    'population': 50339443.0,
    'Unemployment_rate': 9.71,
    'created_date': 3.0,
    'Gross_tertiary_education_enrollment': 55.3
  }, {
    'income': 2800000.0,
    'views': 3303595310.0,
    'category': 'Gaming',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 285700.0,
    'views': 3152402405.0,
    'category': 'People & Blogs',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 5400000.0,
    'views': 3485373675.0,
    'category': 'People & Blogs',
    'population': 51709098.0,
    'Unemployment_rate': 4.15,
    'created_date': 5.0,
    'Gross_tertiary_education_enrollment': 94.3
  }, {
    'income': 1700000.0,
    'views': 8831179714.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1800000.0,
    'views': 6969178081.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 407200.0,
    'views': 1612094871.0,
    'category': 'Entertainment',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 15.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 736300.0,
    'views': 1136534702.0,
    'category': 'Entertainment',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 66900.0,
    'views': 1002219689.0,
    'category': 'Howto & Style',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 25.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 425700.0,
    'views': 1402042328.0,
    'category': 'Gaming',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 22.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 783300.0,
    'views': 6956320454.0,
    'category': 'Education',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 5300000.0,
    'views': 2983799729.0,
    'category': 'Howto & Style',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 1500000.0,
    'views': 3140883140.0,
    'category': 'Travel & Events',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 343600.0,
    'views': 4625777945.0,
    'category': 'Entertainment',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 13.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 2800000.0,
    'views': 4935793409.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 5600000.0,
    'views': 7489455451.0,
    'category': 'Music',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 27.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 11300000.0,
    'views': 10384848759.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 18.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 16800000.0,
    'views': 11552190002.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 8600000.0,
    'views': 11691081301.0,
    'category': 'Entertainment',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 10300000.0,
    'views': 16690788752.0,
    'category': 'Music',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 531800.0,
    'views': 5146004207.0,
    'category': 'Music',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 19.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 630800.0,
    'views': 4465772496.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 20.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 43400000.0,
    'views': 7597013023.0,
    'category': 'Comedy',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 493300.0,
    'views': 2862685032.0,
    'category': 'People & Blogs',
    'population': 126014024.0,
    'Unemployment_rate': 3.42,
    'created_date': 30.0,
    'Gross_tertiary_education_enrollment': 40.2
  }, {
    'income': 38700.0,
    'views': 1113066203.0,
    'category': 'Entertainment',
    'population': 270203917.0,
    'Unemployment_rate': 4.69,
    'created_date': 28.0,
    'Gross_tertiary_education_enrollment': 36.3
  }, {
    'income': 1200000.0,
    'views': 2840137980.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 16.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 460600.0,
    'views': 4021409291.0,
    'category': 'Entertainment',
    'population': 36991981.0,
    'Unemployment_rate': 5.56,
    'created_date': 24.0,
    'Gross_tertiary_education_enrollment': 68.9
  }, {
    'income': 4000000.0,
    'views': 6933660906.0,
    'category': 'Gaming',
    'population': 47076781.0,
    'Unemployment_rate': 13.96,
    'created_date': 29.0,
    'Gross_tertiary_education_enrollment': 88.9
  }, {
    'income': 4600000.0,
    'views': 7683670251.0,
    'category': 'Film & Animation',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1800000.0,
    'views': 7741764747.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 26.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 1700000.0,
    'views': 12607488647.0,
    'category': 'Music',
    'population': 66834405.0,
    'Unemployment_rate': 3.85,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 60.0
  }, {
    'income': 1500000.0,
    'views': 2315226648.0,
    'category': 'Howto & Style',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 10.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 2400000.0,
    'views': 6202090191.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 8.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 8500000.0,
    'views': 4779139505.0,
    'category': 'Education',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 6.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 2400000.0,
    'views': 6993406259.0,
    'category': 'Music',
    'population': 328239523.0,
    'Unemployment_rate': 14.7,
    'created_date': 17.0,
    'Gross_tertiary_education_enrollment': 88.2
  }, {
    'income': 26500000.0,
    'views': 9029609749.0,
    'category': 'Sports',
    'population': 212559417.0,
    'Unemployment_rate': 12.08,
    'created_date': 12.0,
    'Gross_tertiary_education_enrollment': 51.3
  }, {
    'income': 3100000.0,
    'views': 1674409945.0,
    'category': 'People & Blogs',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 14.0,
    'Gross_tertiary_education_enrollment': 28.1
  }, {
    'income': 185800.0,
    'views': 374123483.0,
    'category': 'Gaming',
    'population': 10285453.0,
    'Unemployment_rate': 6.48,
    'created_date': 9.0,
    'Gross_tertiary_education_enrollment': 67.0
  }, {
    'income': 1200000.0,
    'views': 2129773714.0,
    'category': 'Comedy',
    'population': 1366417754.0,
    'Unemployment_rate': 5.36,
    'created_date': 1.0,
    'Gross_tertiary_education_enrollment': 28.1
  }];

  // 选择需要绘制的类别
  var selectedCategories1 = ['Gaming', 'Howto & Style', 'Education'];
  var selectedCategories2 = ['Music', 'Entertainment', 'Education'];
  var selectedCategories3 = ['Music', 'Entertainment', 'Education'];
  var selectedCategories4 = ['Music', 'Entertainment', 'Education'];

  // 筛选出所需类别的数据
  var filteredData1 = data.filter(function (d) {
    return selectedCategories1.includes(d.category);
  });
  var filteredData2 = data.filter(function (d) {
    return selectedCategories2.includes(d.category);
  });
  var filteredData3 = data.filter(function (d) {
    return selectedCategories3.includes(d.category);
  });
  var filteredData4 = data.filter(function (d) {
    return selectedCategories4.includes(d.category);
  });
  var margin = {
    top: 60,
    right: 100,
    bottom: 40,
    left: 90
  };
  var width = 600 - margin.left - margin.right;
  var height = 400 - margin.top - margin.bottom;

  //音乐教育
  var svg = d3.select("#chartContainer1").append("svg").attr("width", width + margin.left + margin.right + 300).attr("height", height + margin.top + margin.bottom + 100).append("g").attr("transform", "translate(".concat(margin.left, ",").concat(margin.top, ")"));
  var xMin = d3.min(filteredData1, function (d) {
    return d.views;
  });
  var xMax = d3.max(filteredData1, function (d) {
    return d.views;
  });
  var yMin = d3.min(filteredData1, function (d) {
    return d.income;
  });
  var yMax = d3.max(filteredData1, function (d) {
    return d.income;
  });
  var xScale = d3.scaleLog().domain([xMin, xMax]).range([0, width]);
  var yScale = d3.scaleLog().domain([yMin, yMax]).range([height, 0]);
  var color = d3.scaleOrdinal(d3.schemeCategory10);
  svg.selectAll(".dot").data(filteredData1).enter().append("circle").attr("class", "dot").attr("cx", function (d) {
    return xScale(d.views);
  }).attr("cy", function (d) {
    return yScale(d.income);
  }).attr("r", 2).style("fill", function (d) {
    return color(d.category);
  });

  // Append x axis
  svg.append("g").attr("transform", "translate(0, ".concat(height, ")")).call(d3.axisBottom(xScale).ticks(5, "~s"));

  // Append x axis label
  svg.append("text").attr("x", width + 50).attr("y", height + margin.top - 55).style("font-size", "12px") // 设置字体大小为 12px
  .attr("text-anchor", "end").text("总播放量");

  // Append y axis
  svg.append("g").call(d3.axisLeft(yScale).ticks(5, "~s"));

  // Append y axis label
  svg.append("text").attr("x", -margin.left + 60).attr("y", -height / 2 + 120).attr("dy", "1em").style("font-size", "12px") // 设置字体大小为 12px
  .attr("text-anchor", "middle").text("年收入/美元");

  // // Create legend items
  // const legendContainer = d3.select("#legend")
  //   .append("svg")
  //   .attr("width", 240)
  //   .attr("height", selectedCategories1.length * 30)
  //   .append("g")
  //   .attr("transform", "translate(0, 20)"); // Move the legend down by 20 units

  // const legendGroups = legendContainer.selectAll("g")
  //   .data(color.domain())
  //   .enter()
  //   .append("g")
  //   .attr("transform", (d, i) => `translate(0, ${i * 20})`);

  // legendGroups.append("circle")
  //   .attr("cx", 5)
  //   .attr("cy", 5)
  //   .attr("r", 5)
  //   .style("fill", d => color(d));

  // legendGroups.append("text")
  //   .attr("x", 20)
  //   .attr("y", 9)
  //   .text(d => d);
  // 创建图例
  var legend = svg.selectAll(".legend").data(color.domain()).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
    return "translate(200, ".concat(i * 20, ")");
  }); // 将图例整体右移 100px

  // 添加图例标记（颜色示例）
  legend.append("rect").attr("x", width - 18).attr("width", 18).attr("height", 18).style("fill", color);

  // 添加图例标签
  legend.append("text").attr("x", width - 24).attr("y", 9).attr("dy", ".35em").style("text-anchor", "end").text(function (d) {
    return d;
  });
}
var _default = exports.default = scatterPlot();
},{}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
var OVERLAY_ID = '__parcel__error__overlay__';
var OldModule = module.bundle.Module;
function Module(moduleName) {
  OldModule.call(this, moduleName);
  this.hot = {
    data: module.bundle.hotData,
    _acceptCallbacks: [],
    _disposeCallbacks: [],
    accept: function (fn) {
      this._acceptCallbacks.push(fn || function () {});
    },
    dispose: function (fn) {
      this._disposeCallbacks.push(fn);
    }
  };
  module.bundle.hotData = null;
}
module.bundle.Module = Module;
var checkedAssets, assetsToAccept;
var parent = module.bundle.parent;
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  var hostname = "" || location.hostname;
  var protocol = location.protocol === 'https:' ? 'wss' : 'ws';
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "65512" + '/');
  ws.onmessage = function (event) {
    checkedAssets = {};
    assetsToAccept = [];
    var data = JSON.parse(event.data);
    if (data.type === 'update') {
      var handled = false;
      data.assets.forEach(function (asset) {
        if (!asset.isNew) {
          var didAccept = hmrAcceptCheck(global.parcelRequire, asset.id);
          if (didAccept) {
            handled = true;
          }
        }
      });

      // Enable HMR for CSS by default.
      handled = handled || data.assets.every(function (asset) {
        return asset.type === 'css' && asset.generated.js;
      });
      if (handled) {
        console.clear();
        data.assets.forEach(function (asset) {
          hmrApply(global.parcelRequire, asset);
        });
        assetsToAccept.forEach(function (v) {
          hmrAcceptRun(v[0], v[1]);
        });
      } else if (location.reload) {
        // `location` global exists in a web worker context but lacks `.reload()` function.
        location.reload();
      }
    }
    if (data.type === 'reload') {
      ws.close();
      ws.onclose = function () {
        location.reload();
      };
    }
    if (data.type === 'error-resolved') {
      console.log('[parcel] ✨ Error resolved');
      removeErrorOverlay();
    }
    if (data.type === 'error') {
      console.error('[parcel] 🚨  ' + data.error.message + '\n' + data.error.stack);
      removeErrorOverlay();
      var overlay = createErrorOverlay(data);
      document.body.appendChild(overlay);
    }
  };
}
function removeErrorOverlay() {
  var overlay = document.getElementById(OVERLAY_ID);
  if (overlay) {
    overlay.remove();
  }
}
function createErrorOverlay(data) {
  var overlay = document.createElement('div');
  overlay.id = OVERLAY_ID;

  // html encode message and stack trace
  var message = document.createElement('div');
  var stackTrace = document.createElement('pre');
  message.innerText = data.error.message;
  stackTrace.innerText = data.error.stack;
  overlay.innerHTML = '<div style="background: black; font-size: 16px; color: white; position: fixed; height: 100%; width: 100%; top: 0px; left: 0px; padding: 30px; opacity: 0.85; font-family: Menlo, Consolas, monospace; z-index: 9999;">' + '<span style="background: red; padding: 2px 4px; border-radius: 2px;">ERROR</span>' + '<span style="top: 2px; margin-left: 5px; position: relative;">🚨</span>' + '<div style="font-size: 18px; font-weight: bold; margin-top: 20px;">' + message.innerHTML + '</div>' + '<pre>' + stackTrace.innerHTML + '</pre>' + '</div>';
  return overlay;
}
function getParents(bundle, id) {
  var modules = bundle.modules;
  if (!modules) {
    return [];
  }
  var parents = [];
  var k, d, dep;
  for (k in modules) {
    for (d in modules[k][1]) {
      dep = modules[k][1][d];
      if (dep === id || Array.isArray(dep) && dep[dep.length - 1] === id) {
        parents.push(k);
      }
    }
  }
  if (bundle.parent) {
    parents = parents.concat(getParents(bundle.parent, id));
  }
  return parents;
}
function hmrApply(bundle, asset) {
  var modules = bundle.modules;
  if (!modules) {
    return;
  }
  if (modules[asset.id] || !bundle.parent) {
    var fn = new Function('require', 'module', 'exports', asset.generated.js);
    asset.isNew = !modules[asset.id];
    modules[asset.id] = [fn, asset.deps];
  } else if (bundle.parent) {
    hmrApply(bundle.parent, asset);
  }
}
function hmrAcceptCheck(bundle, id) {
  var modules = bundle.modules;
  if (!modules) {
    return;
  }
  if (!modules[id] && bundle.parent) {
    return hmrAcceptCheck(bundle.parent, id);
  }
  if (checkedAssets[id]) {
    return;
  }
  checkedAssets[id] = true;
  var cached = bundle.cache[id];
  assetsToAccept.push([bundle, id]);
  if (cached && cached.hot && cached.hot._acceptCallbacks.length) {
    return true;
  }
  return getParents(global.parcelRequire, id).some(function (id) {
    return hmrAcceptCheck(global.parcelRequire, id);
  });
}
function hmrAcceptRun(bundle, id) {
  var cached = bundle.cache[id];
  bundle.hotData = {};
  if (cached) {
    cached.hot.data = bundle.hotData;
  }
  if (cached && cached.hot && cached.hot._disposeCallbacks.length) {
    cached.hot._disposeCallbacks.forEach(function (cb) {
      cb(bundle.hotData);
    });
  }
  delete bundle.cache[id];
  bundle(id);
  cached = bundle.cache[id];
  if (cached && cached.hot && cached.hot._acceptCallbacks.length) {
    cached.hot._acceptCallbacks.forEach(function (cb) {
      cb();
    });
    return true;
  }
}
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","scatterPlot.js"], null)
//# sourceMappingURL=/scatterPlot.0df9c459.js.map