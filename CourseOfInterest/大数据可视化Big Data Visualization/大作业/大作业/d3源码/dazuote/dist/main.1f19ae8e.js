...
function Histogram(data) {
   ...
   */
  var svgg = d3.select("#title").attr("width", 600).attr("height", 30).attr("style", "max-width: 100%; height: auto;");
  svgg.append("g").call(function (g) {
    return g.append("text").attr("x", 0).attr("y", 25).attr("font-size", 20).attr("fill", "currentColor").attr("text-anchor", "start").text("Top Youtuber分区与所在地区社会发展情况的关系");
  });
  ...
  console.log(data);
  var item = data.flatMap(function (_ref) {
    var ii = _ref.ii,
  ...
  console.log(item);
  //var item1=item;
  var item1 = Object.groupBy(item, function (d) {
    return d.ii[0];
  });
  console.log(item1);
  var items = [];
  for (var _i = 0; _i < data.length; _i++) {
    items[_i] = item1[_i].map(function (_ref4) {
      var ii = _ref4.ii,
        key = _ref4.key,
        value = _ref4.value;
      return {
  ...
  for (var _i2 = 0; _i2 < items.length; _i2++) {
    for (var j = 0; j < items[_i2].length; j++) mValue[_i2] = Math.max(mValue[_i2], items[_i2][j].value);
  }
  for (var _i3 = 0; _i3 < items.length; _i3++) {
    for (var _j = 0; _j < items[_i3].length; _j++) items[_i3][_j].value = items[_i3][_j].value / mValue[_i3] * 100;
  }
  var axesDomain = ["Animals(3)", "Autos(3)", "Comedy(51)", "Education(49)", "Entertainment(304)", "Film(42)", "Games(98)", "Howto(36)", "Music(216)", "News(30)", "Nonprofit(2)", "People(101)", "Sports(13)", "Tech(7)"];
  //const axesDomain = items[0].map(d => d.axis)
  //const maxValue =  d3.max(item.map(d => d.value))
  //console.log(maxValue)
  var maxValue = 100;
  var axesLength = items[0].length;
  ...
  var device = function device(d) {
    return ["受高等教育人口百分比", "失业人口百分比", "城镇人口百分比"][d];
  };
  var radarLine = d3.lineRadial().curve(d3.curveCardinalClosed).radius(function (d) {
    return rScale(d);
  }).angle(function (d, i) {
    return i * angleSlice;
  });
  var rScale = d3.scaleLinear().domain([0, maxValue]).range([0, radius]);
  var color = d3.scaleOrdinal().range(["#EDC951", "#CC333F", "#00A0B0", "#003F3F"]);
  var svg = d3.select("#svg1").attr("width", width + 20).attr("height", height).attr("viewBox", [0, 0, width + 20, height]).attr("style", "max-width: 100%; height: auto;");
  var _loop = function _loop(_i4) {
    svg.append("g").call(function (g) {
      return g.append("text").attr("x", 50).attr("y", 30 + 15 * _i4).attr("font-size", 10).attr("fill", "currentColor").attr("text-anchor", "start").text(device(_i4));
    });
    svg.append("g").call(function (g) {
      return g.append("rect").attr("x", 20).attr("y", 20 + 15 * _i4).attr("width", 20).attr("height", 10).attr("fill", color(_i4));
    });
  };
  ...
  svg.append("g").call(function (g) {
    return g.append("text").attr("x", 20).attr("y", 580).attr("font-size", 10).attr("fill", "currentColor").attr("text-anchor", "start").text("注：括号内数值为本类型频道参考youtuber数量");
  });
  var containerWidth = width - margin * 2;
  var containerHeight = height - margin * 2;
  var container = svg.append('g').attr("width", containerWidth).attr("height", containerHeight).attr('transform', "translate(".concat(width / 2, ", ").concat(height / 2, ")"));
  var axisGrid = container.append("g").attr("class", "axisWrapper");
  axisGrid.selectAll(".levels").data(d3.range(1, axisCircles + 1).reverse()).enter().append("circle").attr("class", "gridCircle").attr("r", function (d, i) {
    return radius / axisCircles * d;
  }).style("fill", "#CDCDCD").style("stroke", "#CDCDCD").style("fill-opacity", 0.1);
  var axis = axisGrid.selectAll(".axis").data(axesDomain).enter().append("g").attr("class", "axis");
  ...
  axis.append("text").attr("class", "legend").style("font-size", "11px").attr("text-anchor", "middle").attr("font-family", "monospace").attr("dy", "0.35em").attr("x", function (d, i) {
    return rScale(maxValue * axisLabelFactor) * Math.cos(angleSlice * i - Math.PI / 2);
  }).attr("y", function (d, i) {
    return rScale(maxValue * axisLabelFactor) * Math.sin(angleSlice * i - Math.PI / 2);
  }).text(function (d) {
    return d;
  });
  var plots = container.append('g').selectAll().data(items).join('g').attr("data-name", function (d, i) {
    return device(i);
  ...
  //console.log(plots)
  plots.append('path').attr("d", function (d) {
    return radarLine(d.map(function (v) {
      return v.value;
    }));
  }).attr("fill", function (d, i) {
    return color(i);
  }).attr("fill-opacity", 0.1).attr("stroke", function (d, i) {
    return color(i);
  }).attr("stroke-width", 2);
  plots.selectAll("circle").data(function (d) {
    return d;
  }).join("circle").attr("r", dotRadius).attr("cx", function (d, i) {
...
},{"d3":"../node_modules/d3/index.js"}],"assets/YouTube.csv":[function(require,module,exports) {
module.exports = "/YouTube.1dc519ef.csv";
},{}],"main.js":[function(require,module,exports) {
'use strict';

var _histogram = _interopRequireDefault(require("./histogram.js"));
var d3 = _interopRequireWildcard(require("d3"));
var _YouTube = _interopRequireDefault(require("./assets/YouTube.csv"));
function _getRequireWildcardCache(e) { if ("function" != typeof WeakMap) return null; var r = new WeakMap(), t = new WeakMap(); return (_getRequireWildcardCache = function (e) { return e ? t : r; })(e); }
function _interopRequireWildcard(e, r) { if (!r && e && e.__esModule) return e; if (null === e || "object" != typeof e && "function" != typeof e) return { default: e }; var t = _getRequireWildcardCache(r); if (t && t.has(e)) return t.get(e); var n = { __proto__: null }, a = Object.defineProperty && Object.getOwnPropertyDescriptor; for (var u in e) if ("default" !== u && Object.prototype.hasOwnProperty.call(e, u)) { var i = a ? Object.getOwnPropertyDescriptor(e, u) : null; i && (i.get || i.set) ? Object.defineProperty(n, u, i) : n[u] = e[u]; } return n.default = e, t && t.set(e, n), n; }
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
d3.csv(_YouTube.default).then(function (data, error) {
  if (error) {
    console.log(error);
  } else {
    (0, _histogram.default)(data);
  }
  ;
});
},{"./histogram.js":"histogram.js","d3":"../node_modules/d3/index.js","./assets/YouTube.csv":"assets/YouTube.csv"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
